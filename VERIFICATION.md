...
- Follow-up UI pass: menu open/Escape dismissal, theme toggle/restoration, direct wall-card navigation, AegisLand slider changes/reset, all case-study and résumé routes, and the carousel's idle/drag/arrow/pause states were rechecked. The portfolio origin produced no console errors; the remaining logged messages came from the browser's unrelated extension.
