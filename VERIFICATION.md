...
- The downloaded 3D files are 285,936 bytes (Voyager model) and 225,878 bytes (Jupiter texture). These are measured repository file sizes, not loading-time or bandwidth measurements. The Voyager first-paint fallback is now an attributed NASA/JPL-Caltech archival photograph; the systems cards use real, labeled project evidence rather than CSS-only illustrations.
...
- Image-specific live pass: Chromium loaded the local AbstainBench screenshot (1,348px natural width) and the public TESS transit-recovery plot (2,420px natural width) when the cards entered the viewport. The desktop document width remained 1,348px against a 1,363px viewport, with no horizontal overflow.
